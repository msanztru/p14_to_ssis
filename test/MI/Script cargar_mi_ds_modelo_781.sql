...
IF EXISTS (SELECT 1 FROM dbn1_stg_dhyf.INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='dbo' AND ROUTINE_NAME='spn1_cargar_normalizado_mi_ds_781_cuentas_vivienda' AND ROUTINE_TYPE='PROCEDURE')
DROP PROCEDURE dbo.spn1_cargar_normalizado_mi_ds_781_cuentas_vivienda;
GO

CREATE PROCEDURE dbo.spn1_cargar_normalizado_mi_ds_781_cuentas_vivienda(@logpadre int) AS
BEGIN

	DECLARE @procedure_name varchar(100)='dbn1_stg_dhyf.dbo.spn1_cargar_normalizado_mi_ds_781_cuentas_vivienda'
	SET NOCOUNT ON;
	DECLARE @log int;
