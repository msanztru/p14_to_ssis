...
IF EXISTS (SELECT 1 FROM dbn1_stg_dhyf.INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='dbo' AND ROUTINE_NAME='spn1_cargar_normalizado_mi_ds_848_volumen_operaciones_IAE' AND ROUTINE_TYPE='PROCEDURE')
DROP PROCEDURE dbo.spn1_cargar_normalizado_mi_ds_848_volumen_operaciones_IAE;
GO

CREATE PROCEDURE dbo.spn1_cargar_normalizado_mi_ds_848_volumen_operaciones_IAE(@logpadre int) AS
BEGIN

	DECLARE @procedure_name varchar(100)='dbn1_stg_dhyf.dbo.spn1_cargar_normalizado_mi_ds_848_volumen_operaciones_IAE'
	SET NOCOUNT ON;
	DECLARE @log int;
