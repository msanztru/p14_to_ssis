...
IF EXISTS (SELECT 1 FROM dbn1_stg_dhyf.INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='dbo' AND ROUTINE_NAME='spn1_cargar_normalizado_mi_ds_159_consumo_energia_electrica_2012_full_extract' AND ROUTINE_TYPE='PROCEDURE')
DROP PROCEDURE dbo.spn1_cargar_normalizado_mi_ds_159_consumo_energia_electrica_2012_full_extract;
GO

CREATE PROCEDURE [dbo].[spn1_cargar_normalizado_mi_ds_159_consumo_energia_electrica_2012_full_extract](@p_id_carga int) AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN TRY
		...
		DECLARE	@bd					varchar(50)	= 'dbn1_stg_dhyf',
				...
				@objeto				varchar(200)= 'spn1_cargar_normalizado_mi_ds_159_consumo_energia_electrica_2012_full_extract',
...
from (SELECT * FROM dbn1_stg_dhyf.dbo.tbn1cabe_mf WHERE cabe_modelo='159') cabe
      ...
      INNER JOIN fnn1_query_carga_ds_mi_ds_159_consumo_energia_electrica_2012_f_id_documento(2) doc ON (doc.referencia_larga=CAST(cabe.cabe_ejercicio AS varchar)+'I<'+CAST(cabe.cabe_referdoc AS varchar)+'00K0000')
